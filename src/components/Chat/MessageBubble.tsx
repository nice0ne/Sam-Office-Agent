...
export const MessageBubble: React.FC<MessageBubbleProps> = ({
  ...
  const isThinking = !isUser && !message.content && (!message.toolCalls || message.toolCalls.length === 0);

  return (
    <div className={`flex gap-2 mb-3 group ${isUser ? 'justify-end' : 'justify-start'}`}>
      {!isUser && (
        <div
          className={`w-6 h-6 rounded-full bg-blue-600 text-white flex items-center justify-center shrink-0 mt-1 shadow-sm transition-all ${
